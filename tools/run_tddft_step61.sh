...
# Split the accepted-source VRHO corrector envelope. Diagnostic only.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP61_VRHO_CORRECTOR_01}
...
echo
echo "FPSEID21 STEP61 VRHO CORRECTOR SUMMARY"
...
awk '
  ...
  active && ($2 == "time_step_total" || $2 == "frprmn" ||
             ...
             $2 == "frprmn_vrho_interp" ||
             $2 == "frprmn_vrho_converge" ||
             $2 == "frprmn_vrho_coef_restore") {
    print
    if ($2 == "frprmn_vrho_correct_ctrl") parent=$4
    if ($2 == "frprmn_vrho_interp") interp=$4
    if ($2 == "frprmn_vrho_converge") converge=$4
    if ($2 == "frprmn_vrho_coef_restore") restore=$4
  }
  END {
    printf "derived vrho_corrector_gap %.6f\n", \
           parent-interp-converge-restore
  }
