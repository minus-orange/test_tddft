...
# Split the current off-diagonal envelope. Measurement only.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP73_OFFDIAG_SPLIT_01}
...
echo
echo "FPSEID21 STEP73 OFFDIAG SPLIT SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|energy_offdiag_total|offdiag_hlocal|offdiag_nonloc|offdiag_dot|offdiag_comm_copy|offdiag_gather_output)$/) {
    print
    value[$2]=$4
  }
  END {
    gap=value["energy_offdiag_total"]-value["offdiag_hlocal"] \
        -value["offdiag_nonloc"]-value["offdiag_dot"] \
        -value["offdiag_comm_copy"]-value["offdiag_gather_output"]
    printf "derived offdiag_other %.6f\n", gap
  }
