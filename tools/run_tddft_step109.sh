...
# Split the accepted Step 107 batched SEPPOTF path into GPU stages.
...
echo
echo "FPSEID21 STEP109 BATCHED SEPPOTF SPLIT SUMMARY"
...
echo "FPSEID_SEPPOTF_ACC_SPLIT_BEGIN"
awk '
  /FPSEID_PROFILE_BEGIN/ { active=1; next }
  /FPSEID_PROFILE_END/ { active=0 }
  active && ($2 ~ /^(nonlocf_seppotf|seppotf_mpi|seppotf_acc_project|seppotf_acc_s_batch|seppotf_acc_p_batch|seppotf_acc_final|seppotf_acc_download)$/) {
    print
    count[$2]=$3
    value[$2]=$4
  }
  END {
    ...
    classified=value["seppotf_acc_project"]
    classified+=value["seppotf_acc_s_batch"]
    classified+=value["seppotf_acc_p_batch"]
    classified+=value["seppotf_acc_final"]
    classified+=value["seppotf_acc_download"]
    ...
    if (count["nonlocf_seppotf"] != 202 ||
        ...
        count["seppotf_acc_project"] <= 0 ||
        count["seppotf_acc_s_batch"] <= 0 ||
        count["seppotf_acc_p_batch"] <= 0 ||
        count["seppotf_acc_final"] != 202 ||
        count["seppotf_acc_download"] != 202) {
      print "ERROR: unexpected batched SEPPOTF timer counts." > "/dev/stderr"
      exit 3
    }
    printf "derived classified_sec %.6f\n", classified
    printf "derived unclassified_gap_sec %.6f\n", gap
    printf "derived project_pct %.3f\n",
           100.0*value["seppotf_acc_project"]/parent
    printf "derived s_batch_pct %.3f\n",
           100.0*value["seppotf_acc_s_batch"]/parent
    printf "derived p_batch_pct %.3f\n",
           100.0*value["seppotf_acc_p_batch"]/parent
    printf "derived final_pct %.3f\n",
           100.0*value["seppotf_acc_final"]/parent
    printf "derived download_pct %.3f\n",
           100.0*value["seppotf_acc_download"]/parent
    printf "derived mpi_pct %.3f\n", 100.0*value["seppotf_mpi"]/parent
    printf "derived unclassified_gap_pct %.3f\n", 100.0*gap/parent
  }
' "$ARCHIVE_DIR/tddft.out"
echo "FPSEID_SEPPOTF_ACC_SPLIT_END"
