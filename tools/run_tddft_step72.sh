...
# Split the current expectation/off-diagonal envelope. Measurement only.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP72_EXPECT_SPLIT_01}
...
echo
echo "FPSEID21 STEP72 EXPECTATION SPLIT SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|frprmn_energy_expect|energy_diag_hlocal|energy_diag_nonloc|energy_diag_dot|energy_diag_ee_comm|energy_offdiag_total)$/) {
    print
    value[$2]=$4
  }
  END {
    gap=value["frprmn_energy_expect"]-value["energy_diag_hlocal"] \
        -value["energy_diag_nonloc"]-value["energy_diag_dot"] \
        -value["energy_diag_ee_comm"]-value["energy_offdiag_total"]
    printf "derived energy_expect_other %.6f\n", gap
  }
