...
# Test gang-per-pair, vector-over-G mapping for EWALD G-space.
# Use 01 first; use 02-03 only after a correct, promising first result.
...
do
  label=nvhpc_cufft_1rank_02_STEP99_EWALD_G_GANG_VECTOR_$run_no
...
do
  ...
  echo
  echo "FPSEID21 STEP99 EWALD G-SPACE GANG-VECTOR PERFORMANCE SUMMARY"
  echo "revision=$(git rev-parse HEAD)"
  echo "source_baseline=6ef8676"
  echo "label=$label"
  echo "diagnostic=OFF check=PASS compare=PASS"
  echo "official_step98_median_sec=66.1477772789"
