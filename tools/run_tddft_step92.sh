...
# Check exact nonlocal staging reuse between consecutive TMEVL calls.
# Diagnostic only; equations, device ownership, and the normal build are unchanged.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP92_STEP86_EXNLP_REUSE_01}
...
echo
echo "FPSEID21 STEP92 EXNLP EXACT-REUSE SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|s2_nonlocal|s2_nonlocal_make|s2_nonlocal_gemm|exnlp_gemm_data|exnlp_gemm_dot|exnlp_work1_enter|exnlp_meta_enter)$/) {
    print
  }
  /FPSEID_EXNLP_REUSE_BEGIN/ { reuse=1 }
  reuse { print }
  /FPSEID_EXNLP_REUSE_END/ { reuse=0 }
' "$ARCHIVE_DIR/tddft.out"
echo "equal compares the complete active work2/cfac/ngnl values with the"
echo "previous call of the same Suzuki-Trotter phase."
