...
# Re-profile the accepted Step 52 source. Diagnostic trace only.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP53_STEP52_NSYS_01}
...
DRIVER_LOG=$ROOT_DIR/run/step53_nsys_driver.log
...
echo
echo "FPSEID21 STEP53 NSIGHT SYSTEMS SUMMARY"
