...
# Split VOFRHO on the accepted Step 74 source. Diagnostic only.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP77_VOFRHO_SPLIT_01}
...
echo
echo "FPSEID21 STEP77 VOFRHO SPLIT SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|frprmn_vrho_mix|frprmn_vrho_vofrho|vofrho_xc|vofrho_fft|vofrho_hartree_zero|vofrho_hartree_build|vofrho_hartree_add)$/) {
    print
    value[$2]=$4
  }
  END {
    gap=value["frprmn_vrho_vofrho"]-value["vofrho_xc"] \
      -value["vofrho_fft"]-value["vofrho_hartree_zero"] \
      -value["vofrho_hartree_build"]-value["vofrho_hartree_add"]
    printf "derived vofrho_other %.6f\n", gap
  }
