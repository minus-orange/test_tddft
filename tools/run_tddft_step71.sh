...
# Split the current FRPRMN energy/diagnostic host envelope. Measurement only.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP71_ENERGY_SPLIT_01}
...
echo
echo "FPSEID21 STEP71 ENERGY SPLIT SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|frprmn_energy_diag|frprmn_energy_vg_build|frprmn_energy_efield|frprmn_energy_expect)$/) {
    print
    value[$2]=$4
  }
  END {
    gap=value["frprmn_energy_diag"]-value["frprmn_energy_vg_build"] \
        -value["frprmn_energy_efield"]-value["frprmn_energy_expect"]
    printf "derived frprmn_energy_other %.6f\n", gap
  }
