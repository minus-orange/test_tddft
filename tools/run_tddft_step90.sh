...
# Split the active SEPPOT p channel into its three serial loop stages.
# Diagnostic only; its wall time is not a performance baseline.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP90_STEP86_PCHANNEL_01}
...
echo
echo "FPSEID21 STEP90 SEPPOT P-CHANNEL SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|nonloc_seppot|seppot_extau|seppot_p|seppot_p_projector|seppot_p_reduce|seppot_p_dcoef)$/) {
    print
    value[$2]=$4
  }
  END {
    classified=value["seppot_p_projector"] \
      +value["seppot_p_reduce"]+value["seppot_p_dcoef"]
    printf "derived p_classified_sec %.6f\n", classified
    printf "derived p_gap_sec %.6f\n", value["seppot_p"]-classified
    if (classified > 0.0) {
      printf "derived projector_pct %.3f\n", \
        100.0*value["seppot_p_projector"]/classified
      printf "derived reduce_pct %.3f\n", \
        100.0*value["seppot_p_reduce"]/classified
      printf "derived dcoef_pct %.3f\n", \
        100.0*value["seppot_p_dcoef"]/classified
    }
  }
