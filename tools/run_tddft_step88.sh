...
# Split current NONLOC work without changing equations or ownership.
# Diagnostic only; its wall time is not a performance baseline.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP88_STEP86_NONLOC_01}
...
echo
echo "FPSEID21 STEP88 NONLOC SPLIT SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|tmevl_expectation|frprmn_energy_diag|frprmn_energy_expect|energy_diag_nonloc|offdiag_nonloc|nonloc_kinetic|nonloc_ylm|nonloc_seppot)$/) {
    print
    value[$2]=$4
    count[$2]=$3
  }
  END {
    stages=value["nonloc_kinetic"]+value["nonloc_ylm"] \
      +value["nonloc_seppot"]
    energy=value["energy_diag_nonloc"]+value["offdiag_nonloc"]
    printf "derived nonloc_total_calls %d\n", count["nonloc_kinetic"]
    printf "derived nonloc_stage_total_sec %.6f\n", stages
    printf "derived energy_nonloc_total_sec %.6f\n", energy
    printf "derived kinetic_pct %.3f\n", \
      100.0*value["nonloc_kinetic"]/stages
    printf "derived ylm_pct %.3f\n", 100.0*value["nonloc_ylm"]/stages
    printf "derived seppot_pct %.3f\n", \
      100.0*value["nonloc_seppot"]/stages
  }
' "$ARCHIVE_DIR/tddft.out"
echo "NONLOC stages include TMEVL and energy calls."
