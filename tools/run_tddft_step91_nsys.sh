...
# Re-profile the accepted Step 86 source after the Step 74/80/82/86 changes.
# Diagnostic trace only; its wall time is not a performance baseline.
...
DRIVER_LOG=$ROOT_DIR/run/step91_nsys_driver.log
if ! LABEL="$LABEL" NSYS_TRACE=cuda,openacc,nvtx,osrt,mpi \
    "$SCRIPT_DIR/profile_tddft_nsys.sh" "$RUN_DIR" \
    > "$DRIVER_LOG" 2>&1; then
...
echo
echo "FPSEID21 STEP91 NSIGHT SYSTEMS SUMMARY"
