...
# Build and run the single approved Step 56 Vloc split diagnostic.
# All diagnostic timers are compile-time OFF unless this helper enables them.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP56_VLOC_TIMERS_01}
...
echo
echo "FPSEID21 STEP56 VLOC SPLIT SUMMARY"
...
awk '
  ...
  active && ($2 == "time_step_total" || $2 == "frprmn" ||
             $2 == "tmevl_total" || $2 == "frprmn_vloc_prepare" ||
             $2 == "frprmn_vloc_locpot" ||
             $2 == "frprmn_vloc_smooth_fft") {
    print
    if ($2 == "frprmn_vloc_prepare") parent=$4
    if ($2 == "frprmn_vloc_locpot") locpot=$4
    if ($2 == "frprmn_vloc_smooth_fft") smooth=$4
  }
  END {
    printf "derived frprmn_vloc_other %.6f\n", parent-locpot-smooth
  }
' "$ARCHIVE_DIR/tddft.out"
echo "vloc_other is parent minus LOCPOT minus smoothing/FFT."
