...
# Re-split VRHO on the accepted Step 74 source. Diagnostic only.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP76_STEP74_VRHO_01}
...
echo
echo "FPSEID21 STEP76 CURRENT VRHO SUMMARY"
