...
# Split the current energy HLOCAL envelope without changing equations or data ownership.
# Diagnostic only; its wall time is not a performance baseline.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP85_STEP82_HLOCAL_01}
...
echo
echo "FPSEID21 STEP85 HLOCAL SPLIT SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|frprmn_energy_diag|frprmn_energy_expect|energy_diag_hlocal|offdiag_hlocal|hlocal_zero|hlocal_scatter|hlocal_inverse_fft|hlocal_vg_multiply|hlocal_forward_fft|hlocal_gather)$/) {
    print
    value[$2]=$4
  }
  END {
    children=value["hlocal_zero"] \
      +value["hlocal_scatter"] \
      +value["hlocal_inverse_fft"] \
      +value["hlocal_vg_multiply"] \
      +value["hlocal_forward_fft"] \
      +value["hlocal_gather"]
    parents=value["energy_diag_hlocal"]+value["offdiag_hlocal"]
    printf "derived hlocal_parent_total %.6f\n", parents
    printf "derived hlocal_children_total %.6f\n", children
    printf "derived hlocal_gap %.6f\n", parents-children
  }
