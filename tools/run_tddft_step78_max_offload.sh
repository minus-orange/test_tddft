...
# One-run, temporary upper-bound experiment.  The source commit is reverted
# after its A100 result is recorded; it never replaces the Step 74 baseline.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP78_MAX_OFFLOAD_01}
...
echo
echo "FPSEID21 STEP78 TEMPORARY MAX OFFLOAD SUMMARY"
echo "revision=$(git rev-parse HEAD)"
echo "label=$LABEL"
echo "diagnostic=OFF check=PASS compare=PASS"
echo "official_step74_median_sec=68.0681188811"
...
echo "Temporary one-run experiment; return one photo of this summary."
echo "Do not run 02/03.  The implementation will be reverted."
