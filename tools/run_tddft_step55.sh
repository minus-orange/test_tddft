...
# Build and run the single approved Step 55 VRHO split diagnostic.
...
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP55_VRHO_TIMERS_01}
...
echo
echo "FPSEID21 STEP55 VRHO SPLIT SUMMARY"
...
awk '
  ...
  active && ($0 ~ /time_step_total|frprmn[[:space:]]|tmevl_total|frprmn_vrho_mix[[:space:]]|frprmn_vrho_vofrho|frprmn_vrho_smooth_fft|frprmn_vrho_mix_control/) { print }
' "$ARCHIVE_DIR/tddft.out"
echo "VRHO child rows partition frprmn_vrho_mix."
