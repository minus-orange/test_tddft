...
# Split current ELECTF LOCPOTF into its local-build/MPI envelope and remainder.
# Diagnostic only; equations, loops, MPI, ownership, and normal builds unchanged.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP94_STEP86_LOCPOTF_01}
...
echo
echo "FPSEID21 STEP94 ELECTF LOCPOTF SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|electf_force|electf_locpotf_total|locpotf_local_mpi)$/) {
    print
    value[$2]=$4
  }
  END {
    ...
    if (total > 0.0) {
      printf "derived local_mpi_pct %.3f\n", 100.0*local/total
      printf "derived remainder_pct %.3f\n", 100.0*(total-local)/total
    }
  }
' "$ARCHIVE_DIR/tddft.out"
echo "local_mpi includes local G construction, force accumulation, and MPI."
