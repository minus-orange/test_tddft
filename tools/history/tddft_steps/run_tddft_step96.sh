...
# Diagnose exact reuse of fixed-nuclei EWALDY EWA and FORCE outputs.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP96_STEP86_EWALD_REUSE_01}
...
echo
echo "FPSEID21 STEP96 EWALD EXACT-REUSE SUMMARY"
...
awk '
  /FPSEID_EWALD_REUSE_BEGIN/ { active=1 }
  active { print }
  /FPSEID_EWALD_REUSE_END/ { active=0 }
' "$ARCHIVE_DIR/tddft.out"
echo "Exact comparisons are against the preceding EWALDY call."
