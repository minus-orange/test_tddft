...
# Split exact reuse into ngnl, cfac, and work2 components.
# Diagnostic only; equations, device ownership, and normal builds are unchanged.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP93_STEP86_EXNLP_PARTS_01}
...
echo
echo "FPSEID21 STEP93 EXNLP COMPONENT-REUSE SUMMARY"
...
awk '
  /FPSEID_EXNLP_COMPONENT_BEGIN/ { show=1 }
  show { print }
  /FPSEID_EXNLP_COMPONENT_END/ { show=0 }
' "$ARCHIVE_DIR/tddft.out"
echo "Percentages compare each component with the preceding call of the"
echo "same Suzuki-Trotter phase.  Diagnostic wall is not a baseline."
