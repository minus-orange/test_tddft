...
# Re-profile the accepted Step 99 source and rank the major remaining timers.
...
echo
echo "FPSEID21 STEP100 CURRENT-SOURCE TIMER SUMMARY"
echo "revision=$(git rev-parse HEAD)"
echo "source_baseline=6b4099f"
echo "label=$LABEL"
echo "diagnostic=ON check=PASS compare=PASS"
echo "official_step99_median_sec=64.3024969101"
grep 'steps took' "$ARCHIVE_DIR/tddft.out" | tail -n 1
echo "FPSEID_STEP100_TIMERS_BEGIN"
awk '
  /FPSEID_PROFILE_BEGIN/ { active=1; next }
  /FPSEID_PROFILE_END/ { active=0 }
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|tmevl_s2|s2_nonlocal|s2_nonlocal_make|s2_nonlocal_gemm|exnlp_gemm_dot|electf_force|electf_locpotf_total|locpotf_ewald|ewald_g_space|ewald_r_space|frprmn_part1to5|frprmn_vrho_mix|frprmn_extau_prepare|frprmn_energy_diag|hlocal_acc_total)$/) {
    print
  }
' "$ARCHIVE_DIR/tddft.out"
echo "FPSEID_STEP100_TIMERS_END"
