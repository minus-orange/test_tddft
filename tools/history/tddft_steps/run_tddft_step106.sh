...
# Split current-source ELECTF SEPPOTF into phase generation, tutorial s/p
# projector generation, tutorial s/p band reductions, MPI, and a gap.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP106_STEP102_SEPPOTF_DETAIL_01}
...
echo
echo "FPSEID21 STEP106 ELECTF SEPPOTF DETAIL SUMMARY"
...
echo "FPSEID_SEPPOTF_DETAIL_BEGIN"
awk '
  /FPSEID_PROFILE_BEGIN/ { active=1; next }
  /FPSEID_PROFILE_END/ { active=0 }
  active && ($2 ~ /^(nonlocf_seppotf|seppotf_phase|seppotf_s_projector|seppotf_s_band_reduce|seppotf_p_projector|seppotf_p_band_reduce|seppotf_mpi)$/) {
    print
    count[$2]=$3
    value[$2]=$4
  }
  END {
    parent=value["nonlocf_seppotf"]
    classified=value["seppotf_phase"]
    classified+=value["seppotf_s_projector"]
    classified+=value["seppotf_s_band_reduce"]
    classified+=value["seppotf_p_projector"]
    classified+=value["seppotf_p_band_reduce"]
    classified+=value["seppotf_mpi"]
    gap=parent-classified
    if (parent <= 0.0) {
      print "ERROR: SEPPOTF parent timer is missing." > "/dev/stderr"
      exit 2
    }
    if (count["nonlocf_seppotf"] != 202 ||
        count["seppotf_phase"] <= 0 ||
        count["seppotf_s_projector"] <= 0 ||
        count["seppotf_s_band_reduce"] <= 0 ||
        count["seppotf_p_projector"] <= 0 ||
        count["seppotf_p_band_reduce"] <= 0 ||
        count["seppotf_mpi"] != 202) {
      print "ERROR: unexpected SEPPOTF timer counts." > "/dev/stderr"
      exit 3
    }
    printf "derived classified_sec %.6f\n", classified
    printf "derived unclassified_gap_sec %.6f\n", gap
    printf "derived phase_pct %.3f\n", 100.0*value["seppotf_phase"]/parent
    printf "derived s_projector_pct %.3f\n",
           100.0*value["seppotf_s_projector"]/parent
    printf "derived s_band_reduce_pct %.3f\n",
           100.0*value["seppotf_s_band_reduce"]/parent
    printf "derived p_projector_pct %.3f\n",
           100.0*value["seppotf_p_projector"]/parent
    printf "derived p_band_reduce_pct %.3f\n",
           100.0*value["seppotf_p_band_reduce"]/parent
    printf "derived mpi_pct %.3f\n", 100.0*value["seppotf_mpi"]/parent
    printf "derived unclassified_gap_pct %.3f\n", 100.0*gap/parent
  }
' "$ARCHIVE_DIR/tddft.out"
echo "FPSEID_SEPPOTF_DETAIL_END"
