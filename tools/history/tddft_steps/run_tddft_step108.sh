...
# Re-profile the accepted Step 107 source and rank the major remaining timers.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP108_STEP107_CURRENT_PROFILE_01}
...
echo
echo "FPSEID21 STEP108 CURRENT-SOURCE TIMER SUMMARY"
...
echo "FPSEID_STEP108_TIMERS_BEGIN"
awk '
  /FPSEID_PROFILE_BEGIN/ { active=1; next }
  /FPSEID_PROFILE_END/ { active=0 }
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|tmevl_s2|s2_nonlocal|s2_nonlocal_make|s2_nonlocal_gemm|exnlp_gemm_dot|frprmn_part1to5|frprmn_extau_prepare|frprmn_vrho_mix|frprmn_energy_diag|electf_force|electf_locpotf_total|electf_nonlocf_total|nonlocf_kinetic_mpi|nonlocf_seppotf|hlocal_acc_total|locpotf_ewald|ewald_g_space|ewald_r_space)$/) {
    print
  }
' "$ARCHIVE_DIR/tddft.out"
echo "FPSEID_STEP108_TIMERS_END"
