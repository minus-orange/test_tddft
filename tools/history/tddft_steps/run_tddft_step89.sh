...
echo
echo "FPSEID21 STEP89 SEPPOT CHANNEL SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|frprmn_energy_expect|nonloc_seppot|seppot_extau|seppot_s|seppot_p|seppot_d|seppot_f)$/) {
    print
    value[$2]=$4
    count[$2]=$3
  }
  END {
    channels=value["seppot_s"]+value["seppot_p"] \
      +value["seppot_d"]+value["seppot_f"]
    classified=value["seppot_extau"]+channels
    printf "derived seppot_classified_sec %.6f\n", classified
    printf "derived seppot_gap_sec %.6f\n", \
      value["nonloc_seppot"]-classified
    if (classified > 0.0) {
      printf "derived extau_pct %.3f\n", \
        100.0*value["seppot_extau"]/classified
      printf "derived s_pct %.3f\n", 100.0*value["seppot_s"]/classified
      printf "derived p_pct %.3f\n", 100.0*value["seppot_p"]/classified
      printf "derived d_pct %.3f\n", 100.0*value["seppot_d"]/classified
      printf "derived f_pct %.3f\n", 100.0*value["seppot_f"]/classified
    }
  }
' "$ARCHIVE_DIR/tddft.out"
echo "SEPPOT stages include TMEVL and energy calls."
