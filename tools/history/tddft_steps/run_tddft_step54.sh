...
# Build and run the single approved Step 54 FRPRMN host-envelope diagnostic.
# All added timers are compile-time OFF unless this helper enables them.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP54_FRPRMN_HOST_TIMERS_01}
...
echo
echo "FPSEID21 STEP54 FRPRMN HOST-ENVELOPE SUMMARY"
...
awk '
  ...
  active && ($0 ~ /time_step_total|frprmn[[:space:]]|tmevl_total|frprmn_rhoofk|frprmn_rhoget|frprmn_coef_sync|frprmn_coef_setup|frprmn_gdump_prepare|frprmn_part1to5|frprmn_extau_prepare|frprmn_vloc_prepare|frprmn_vrho_mix|frprmn_energy_diag|frprmn_initial_density|frprmn_iter_init|frprmn_pre_tmevl|frprmn_post_tmevl|frprmn_density_init|frprmn_exit_cleanup/) { print }
