...
# Re-measure broad FRPRMN envelopes on the accepted Step 74 source.
# This diagnostic changes no equations or OpenACC ownership.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP75_STEP74_FRPRMN_01}
...
echo
echo "FPSEID21 STEP75 CURRENT FRPRMN SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|frprmn_rhoofk|frprmn_rhoget|frprmn_coef_sync|frprmn_coef_setup|frprmn_gdump_prepare|frprmn_part1to5|frprmn_extau_prepare|frprmn_vloc_prepare|frprmn_vrho_mix|frprmn_energy_diag)$/) {
    print
    value[$2]=$4
  }
  END {
    residual=value["frprmn"]-value["tmevl_total"]
    classified=value["frprmn_rhoofk"]+value["frprmn_rhoget"] \
      +value["frprmn_coef_sync"]+value["frprmn_coef_setup"] \
      +value["frprmn_gdump_prepare"]+value["frprmn_part1to5"] \
      +value["frprmn_extau_prepare"]+value["frprmn_vloc_prepare"] \
      +value["frprmn_vrho_mix"]+value["frprmn_energy_diag"]
    printf "derived frprmn_residual %.6f\n", residual
    printf "derived frprmn_unclassified %.6f\n", residual-classified
  }
' "$ARCHIVE_DIR/tddft.out"
echo "Current accepted Step 74 source; diagnostic wall is not a baseline."
