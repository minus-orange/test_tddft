...
# Measure the accepted Step 86 device-resident HLOCAL path.
...
echo
echo "FPSEID21 STEP87 CURRENT HLOCAL SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|frprmn_energy_diag|frprmn_energy_expect|energy_diag_hlocal|offdiag_hlocal|hlocal_acc_total)$/) {
    print
    value[$2]=$4
    count[$2]=$3
  }
  END {
    tmevl=value["hlocal_acc_total"] \
      -value["energy_diag_hlocal"] \
      -value["offdiag_hlocal"]
    reduction=0.482563-value["hlocal_acc_total"]
    printf "derived hlocal_total_calls %d\n", count["hlocal_acc_total"]
    printf "derived hlocal_tmevl_sec %.6f\n", tmevl
    printf "derived step85_hlocal_sec 0.482563\n"
    printf "derived hlocal_reduction_sec %.6f\n", reduction
    printf "derived hlocal_reduction_pct %.3f\n", \
      100.0*reduction/0.482563
  }
