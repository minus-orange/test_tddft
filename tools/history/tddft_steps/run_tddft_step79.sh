...
# Split G2VXC2 on the accepted Step 74 source. Diagnostic only.

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../../.." && pwd)
RUN_DIR=${RUN_DIR:-"$ROOT_DIR/run/Si111-H_nvhpc"}
LABEL=${LABEL:-nvhpc_cufft_1rank_02_STEP79_XC_SPLIT_01}
...
echo
echo "FPSEID21 STEP79 XC SPLIT SUMMARY"
...
awk '
  ...
  active && ($2 ~ /^(time_step_total|frprmn|tmevl_total|frprmn_vrho_vofrho|vofrho_xc|g2vxc_derivative_setup|g2vxc_derivative_fft|g2vxc_exchange|g2vxc_correlation|g2vxc_assemble)$/) {
    print
    value[$2]=$4
  }
  END {
    gap=value["vofrho_xc"]-value["g2vxc_derivative_setup"] \
      -value["g2vxc_derivative_fft"]-value["g2vxc_exchange"] \
      -value["g2vxc_correlation"]-value["g2vxc_assemble"]
    printf "derived g2vxc_other %.6f\n", gap
  }
