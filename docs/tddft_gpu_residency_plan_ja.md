...
run 01は両checkにPASSし`68.4441161156`秒でしたが、改善`0.188650%`はbaseline実行幅内
です。run 02/03は必須です。

run 02/03も両checkにPASSしました。中央値`68.3616518974`秒はStep 62より`0.308907%`
高速で実測範囲も重ならないため、Step 67を新baselineとして正式採用します。

Step 68は同じ限定的探索の次点としてVPJ vector length 64だけを試し、3回gate通過までは
Step 67を維持します。
