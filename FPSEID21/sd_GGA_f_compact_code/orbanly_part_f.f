      subroutine orbanly(iband,ik,coef,rho1,rho2,ng2q,ng2
...
c ****  temp check end
      ...
      do 2 i=1,mesh
    2 work(i)=phil(i,l)**2
   13 call normint(rc(l,ity),rad,work,mesh,meshq,pssum(l),l
     &            ,H,zo(l,ity))
...
c ****
      subroutine normint(rc,rad,work,mesh,meshq,sum2,l,H,zo)
      implicit double precision (a-h,o-z)
      dimension rad(meshq),work(meshq)
      sum=rint(rad,work,mesh,H)
      if ( l.eq.1 ) write(6,1000)sum
      if ( l.eq.2) write(6,2000)sum
      if ( l.eq.3) write(6,3000)sum
      if ( l.eq.4) write(6,4000)sum
      sum2=rcint(rc,rad,work,mesh,H)
      if (l.eq.1 ) write(6,1100)sum2,rc
      if (l.eq.2 ) write(6,1200)sum2,rc
      if (l.eq.3 ) write(6,1300)sum2,rc
      if (l.eq.4 ) write(6,1400)sum2,rc
