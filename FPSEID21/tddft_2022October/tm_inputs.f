...
C**************************************************************
     ...
     &                  NFL, NPFL, RCUT,
     &                  GCUT2,GRAT, KCONT,MXBND,dt,ntstep)
     ...
     &          BSAVE(80)*20,A1(72)*1,NAIBU*80
C
      call MPI_COMM_RANK(MPI_COMM_WORLD,my_rank,ierr)
c **** temp check
c      write(6,*)' my_rank=',my_rank,' sub. AINPUT is working'
c **** temp check ; end
      NFIX = 0
      NUL=' '
      DO 2100 I=1,80
...
C****************************************************
      ...
      call MPI_Bcast(A1,72,MPI_CHARACTER,0,MPI_COMM_WORLD,ierr)
      if ( my_rank.eq.0 ) then
      WRITE(6,1) A1
      endif
      I=0
 9000 if ( my_rank.eq.0 ) READ(5,1) A1
      call MPI_Bcast(A1,72,MPI_CHARACTER,0,MPI_COMM_WORLD,ierr)
    1 FORMAT(72A1)
      N=1
...
C
 ...
 1000 CONTINUE
  201 JSAVE=L
C
      if ( my_rank.eq.0 ) WRITE(6,*) ' # OF COMMANDS = ',JSAVE
      DO 220 I=1,JSAVE
      if (my_rank.eq.0 ) WRITE(6,221) I,ASAVE(I),BSAVE(I)
  220 continue
...
C
      ...
      ELSEIF(ASAVE(L)(1:2).EQ.'CG') THEN
            ...
            call MPI_Bcast(ITC7  ,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
            if (my_rank.eq.0 ) then
            WRITE(NAIBU,'(A20)') BSAVE(L)(I1+1:I2-1)
            READ(NAIBU,'(BN,I20)') ITC1
            endif
            call MPI_Bcast(ITC1  ,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
            if (my_rank.eq.0 ) then
            WRITE(NAIBU,'(A20)') BSAVE(L)(I2+1:I9)
            READ(NAIBU,'(BN,I20)') ITCMAX
            endif
            call MPI_Bcast(ITCMAX,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
            if (my_rank.eq.0 ) then
      ...
      ELSEIF(ASAVE(L)(1:6).EQ.'SUZUKI') THEN
         if ( my_rank.eq.0 ) then
          WRITE(NAIBU,'(A20)') BSAVE(L)
          READ(NAIBU,'(BN,I20)') ioption
         endif
         call MPI_Bcast(ioption,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         if ( ioption.eq.2 ) then
           if ( my_rank.eq.0 ) then
           write(6,*)' Second order Suzuki-Trotter decomposition!! '
           endif
         elseif ( ioption.eq.3 ) then
           if ( my_rank.eq.0 ) then
           write(6,*)' Fourth order Suzuki-Trotter decomposition
     &  (Ver.1)!! '
           endif
     ...
     &  (Ver.2)!! '
           ...
           if ( my_rank.eq.0 ) then
           write(6,*)' WRONG option for Suzuki-Trotter decompostion'
           write(6,*)'    has been assigned !!!  STOPPING '
           endif
           stop
         endif
      ELSEIF(ASAVE(L)(1:3).EQ.'EXT') THEN
         ...
         ENDIF
      ELSEIF(ASAVE(L)(1:4).EQ.'TMOD') THEN
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,I10)') ITMOD
         endif
         call MPI_Bcast(ITMOD,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
         WRITE(6,*) ' FREQUENCY OF MONITOR ',ITMOD
         endif
      ELSEIF(ASAVE(L)(1:4).EQ.'RMIX') THEN
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,E20.0)') RMIX
         endif
         call MPI_Bcast(RMIX ,1,MPI_DOUBLE_PRECISION
     &     ,0,MPI_COMM_WORLD,ierr)
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'CONV') THEN
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,E20.0)') TR2
         endif
         call MPI_Bcast(TR2,1,MPI_DOUBLE_PRECISION
     &    ,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'MAXF') THEN
         ...
         call MPI_Bcast(MAXFN,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'KCON') THEN
         ...
         call MPI_Bcast(KCONT,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'OKST') THEN
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,E20.0)') OKSTEP
         endif
         call MPI_Bcast(OKSTEP,1,MPI_DOUBLE_PRECISION
     &                ,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'ALAT') THEN
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,E20.0)') CELLDM(1)
         endif
         call MPI_Bcast(CELLDM(1),1,MPI_DOUBLE_PRECISION
     &     ,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'ZVAL') THEN
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,E20.0)') ZVAL
         endif
         call MPI_Bcast(ZVAL,1,MPI_DOUBLE_PRECISION
     &    ,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'DUAL') THEN
         ...
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,E20.0)') GRAT
         endif
         call MPI_Bcast(GRAT,1,MPI_DOUBLE_PRECISION
     &    ,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'RCUT') THEN
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,E20.0)') RCUT
         endif
         call MPI_Bcast(RCUT,1,MPI_DOUBLE_PRECISION
     &    ,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
      ...
      ELSEIF(ASAVE(L)(1:4).EQ.'GCUT') THEN
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)
         READ(NAIBU,'(BN,E20.0)') GCUT2
         endif
         call MPI_Bcast(GCUT2,1,MPI_DOUBLE_PRECISION
     &    ,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
...
c **** Miyamoto
      ...
      elseif(asave(l)(1:2).eq.'dt') then
       if ( my_rank.eq.0 ) then
       write(naibu,'(A20)') bsave(l)
       read( naibu,'(BN,E20.12)') dt
       endif
       call MPI_Bcast(dt,1,MPI_DOUBLE_PRECISION
     &   ,0,MPI_COMM_WORLD,ierr)
       if ( my_rank.eq.0 ) then
       write(6,*)' Delta t = ',dt,' a.u.'
       endif
      elseif(asave(l)(1:5).eq.'tstep') then
       if ( my_rank.eq.0 ) then
       write(naibu,'(A20)') bsave(l)
       read( naibu,'(BN,I20)')ntstep 
       endif
...
C
      ELSEIF(ASAVE(L)(1:5).EQ.'METAL') THEN
         IF(INDEX(BSAVE(L),',').NE.0) THEN
            ...
            call MPI_Bcast(NFL,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
            I1=INDEX(BSAVE(L),',')+1
            I2=INDEX(BSAVE(L),')')-1
            if ( my_rank.eq.0 ) then
            WRITE(NAIBU,'(A20)') BSAVE(L)(I1:I2)
            ...
            call MPI_Bcast(NPFL,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         ELSE
            if ( my_rank.eq.0 ) then
...
C
      ELSEIF(ASAVE(L)(1:5).EQ.'FORCE') THEN
         ...
         call MPI_Bcast(IFATM(NFIX),1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)(I1+1:I2-1)
         READ(NAIBU,'(BN,F20.0)') XX
         endif
         call MPI_Bcast(XX,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)(I2+1:I3-1)
         READ(NAIBU,'(BN,F20.0)') YY
         endif
         call MPI_Bcast(YY,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
         WRITE(NAIBU,'(A20)') BSAVE(L)(I3+1:I9)
         READ(NAIBU,'(BN,F20.0)') ZZ
         endif
         call MPI_Bcast(ZZ,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
     ...
     &               NFIX,IFATM(NFIX),XX,YY,ZZ
      ...
      ELSEIF(ASAVE(L)(1:5).EQ.'MAXG2') THEN
         ...
         call MPI_Bcast(MAXG2,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         if ( my_rank.eq.0 ) then
...
C     SETS UP THE BASIS VECTORS AND INITIAL GEOMETRY.
     ...
     &  NTAUQ,NTYQ,TAU,NTYPE,NUMTY,NIDN, MXOFL, COVA, ALAT,atmss )
C
      if ( my_rank.eq.0 ) then
...
c ***  Miyamoto 
  ...
  199  continue
       ...
       nsize=3*3*48
       call MPI_Bcast(S,nsize,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      ELSE
        if ( my_rank.eq.0 ) then
...
C *******
 ...
 7000   WK(IK) = OMEGA * WGT(IK)
      ...
      if ( my_rank.eq.0 ) write(6,*) ' begin LVGENX! '
        CALL LVGENX(A1,A2,A3,S,NTOT,RKK,KG,KZ,NSY,NEXPND)
c      if ( my_rank.eq.0 ) write(6,*) ' LVGENX finished ! '
c      write(6,*)' my_rank=',my_rank,' LVGENX finished ! '
        CALL MESHK(S,NTOT,NUMK,NKMESH,KG,NEXPND,MM,RCOSIN,JDR,SK,
     &           WK,NJD,CCO, NDX, NDY, NDZ )
      if ( my_rank.eq.0 ) write(6,*) ' MESHK finished ! '
C
CARE ****
c **** Miyamoto 9/6/95
c        read(5,*)nexpnd
...
C **
      ...
      ELSEIF(IOPT(2,2).EQ.2) THEN
         if ( my_rank.eq.0 ) then
          write(6,*)' NUMK=2 IS NOT SUPPORTED'
         endif
         STOP
      ...
      ELSEIF(IOPT(2,2).EQ.5) THEN
         ...
         WK( 4)=1.D0/4.D0
      ENDIF
         IF(NUMK.LE.0.OR.NUMK.GT.NUMKQ) then
          if ( my_rank.eq.0 ) write(6,*)' NUMK'
          STOP
         ENDIF
C
      if ( my_rank.eq.0 ) then
...
C**********************************************************
     ...
     &  NTAUQ,NTYQ,
     &  TAU,NTYPE,NUMTY,NIDN, MXOFL, COVA, ALAT,atmss )
      IMPLICIT REAL*8 (A-H,O-Z)
      include 'mpif.h'
C
...
C
      ...
      ENDIF
C
          IF( ZVAL.LT.0.1D00 ) then
           if ( my_rank.eq.0 ) write(6,*)' **  STRUC:  ZVAL = 0 ???'
           STOP
          ENDIF
          NBND =  ZVAL + 0.1D-04
  ...
  120               OCC(I,K)=0.D0
          ...
          DO 122 I=1,NBND
  122     OCC(I,K)=1.D0
          IF( IFLAG .EQ. 1) OCC(NBND,K) = 0.5D+00
  121     CONTINUE
C
C
...
CARE  **  ALPHA QUARTZ or CRISTOBALITE
        ...
        NUMTY(2) = 2 * NUMTY(1)
          ...
          IF( NUMCHK .NE. NTAUQ ) THEN
             if ( my_rank.eq.0 ) WRITE(6,801) NUMTY(1), NUMTY(2), NTAUQ
  ...
  812   NIDN(I,2) = I + NUMTY(1)
        ...
        call MPI_Bcast(COVA,1,MPI_DOUBLE_PRECISION
     &   ,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(U   ,1,MPI_DOUBLE_PRECISION
     &   ,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(X   ,1,MPI_DOUBLE_PRECISION
     &   ,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(Y   ,1,MPI_DOUBLE_PRECISION
     &   ,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(Z   ,1,MPI_DOUBLE_PRECISION
     &   ,0,MPI_COMM_WORLD,ierr)
          if( iopt(1,2) .eq. 2 ) then
        A1(1) =                    0.5D+00 * ALAT
...
cccc        read(5,*) alat   ! read in AINPUT
        ...
        call MPI_Bcast(avec,3*3,MPI_DOUBLE_PRECISION
     &    ,0,MPI_COMM_WORLD,ierr)
        do 991 i=1,3
         a1(i)=alat*avec(i,1)
         a2(i)=alat*avec(i,2)
         a3(i)=alat*avec(i,3)
  991   continue
        if ( my_rank.eq.0 ) read(5,*)ntype
        call MPI_Bcast(ntype,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
        ntseq=0
        do 992 ity=1,ntype
         if ( my_rank.eq.0 ) then
         read(5,*)numty(ity),mxofl(ity),amass
         endif
         call MPI_Bcast(numty(ity),1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         call MPI_Bcast(mxofl(ity),1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
         call MPI_Bcast(amass     ,1,MPI_DOUBLE_PRECISION
     &    ,0,MPI_COMM_WORLD,ierr)
          do 993 it=1,abs(numty(ity))
           ...
           call MPI_Bcast(rat,3,MPI_DOUBLE_PRECISION 
     &      ,0,MPI_COMM_WORLD,ierr)
...
C   AL
  ...
  721   NIDN(I,1)=I
        ...
        call MPI_Bcast(ALAT,1,MPI_DOUBLE_PRECISION
     &     ,0,MPI_COMM_WORLD,ierr)
        if ( my_rank.eq.0 ) then
        WRITE(6,200) ALAT
        endif
...
C
        ...
        call MPI_Bcast(IOP, 3*3,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(AVEC,3*3,MPI_DOUBLE_PRECISION
     &   ,0,MPI_COMM_WORLD,ierr)
        DO 10 J=1,3
        DO 10 I=1,3
   ...
   10   CONTINUE
        DO 11 I=1,3
          ...
          A3(I)=AVEC(I,3)
   11   CONTINUE
        if ( my_rank.eq.0 ) READ(5,*) NUMAT
        call MPI_Bcast(NUMAT,1,MPI_INTETGER,0,MPI_COMM_WORLD,ierr)
        ...
        DO 12 JA=1,NTAUQ
          ...
          call MPI_Bcast(JOP,3,MPI_INTEGER  ,0,MPI_COMM_WORLD,ierr)
          call MPI_Bcast(RAT,3,MPI_DOUBLE_PRECISION
     &        ,0,MPI_COMM_WORLD,ierr)
          call MPI_Bcast( IP,1,MPI_CHARACTER,0,MPI_COMM_WORLD,ierr)
          DO 13 I=1,3
            SGN=UM
   ...
   13     CONTINUE
          IF(IP.NE.' ') THEN
            ...
            call MPI_Bcast(JOP,3,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
            call MPI_Bcast(DRAT,3,MPI_DOUBLE_PRECISION
     &       ,0,MPI_COMM_WORLD,ierr)
            DO 14 I=1,3
              SGN=UM
   ...
   12   CONTINUE
        if ( my_rank.eq.0 ) READ(5,*) IBRAV,PGIND
        call MPI_Bcast(IBRAV,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(PGIND,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
C
...
C                 A3(3) =   3.5D+00 * ALAT
           if ( my_rank.eq.0 )READ(5,*) A3(3)
           call MPI_Bcast(A3(3),1,MPI_DOUBLE_PRECISION
     &      ,0,MPI_COMM_WORLD,ierr)
           A3(3) = A3(3) * ALAT
C
        if ( my_rank.eq.0 ) then
