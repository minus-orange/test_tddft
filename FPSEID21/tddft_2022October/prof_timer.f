...
#endif
      common /profdata/ pt(151),pt0(151),pc(151),prank
#ifdef FPSEID_FRPRMN_DIAGNOSTIC
   ...
   20 continue
      ...
      integer pc,prank
      common /profdata/ pt(151),pt0(151),pc(151),prank
      if (id.lt.1 .or. id.gt.151) return
      pt0(id)=MPI_Wtime()
      ...
      character*24 name
      common /profdata/ pt(151),pt0(151),pc(151),prank
      dimension psum(151),pmax(151),pcmax(151)
      call MPI_Reduce(pt,psum,151,MPI_DOUBLE_PRECISION,MPI_SUM,0,
     &                MPI_COMM_WORLD,ierr)
      call MPI_Reduce(pt,pmax,151,MPI_DOUBLE_PRECISION,MPI_MAX,0,
     &                MPI_COMM_WORLD,ierr)
      call MPI_Reduce(pc,pcmax,151,MPI_INTEGER,MPI_MAX,0,
     &                MPI_COMM_WORLD,ierr)
  ...
  100 format(1x,i3,1x,a24,1x,i10,2(1x,f18.6))
      ...
      if (id.eq.121)name='locpotf_ewald'
      if (id.eq.122)name='locpotf_local_energy'
      if (id.eq.123)name='locpotf_xc'
      if (id.eq.124)name='locpotf_hartree'
      if (id.eq.125)name='ewald_g_space'
      if (id.eq.126)name='ewald_r_space'
      if (id.eq.127)name='ewald_mpi'
      if (id.eq.128)name='electf_nonlocf_total'
      if (id.eq.129)name='nonlocf_setup'
      if (id.eq.130)name='nonlocf_kinetic_mpi'
      if (id.eq.131)name='nonlocf_getylm'
      if (id.eq.132)name='nonlocf_seppotf'
      if (id.eq.133)name='nonlocf_finalize'
      if (id.eq.134)name='seppotf_phase'
      if (id.eq.135)name='seppotf_s_projector'
      if (id.eq.136)name='seppotf_s_band_reduce'
      ...
      if (id.eq.145)name='nonlocf_k_gprep'
      if (id.eq.146)name='nonlocf_k_reduce'
      if (id.eq.147)name='nonlocf_k_comm'
      if (id.eq.148)name='nonlocf_eed_gprep'
      if (id.eq.149)name='nonlocf_eed_reduce'
      if (id.eq.150)name='nonlocf_eed_comm'
      if (id.eq.151)name='nonlocf_ylm_radius'
      return
      end
